...
w = 1;  % collector width (m)
al = 2; % collector length (m)
ep = 0.95;  % emissivity of underside of absorber plate 
eb = 0.95;  % emissivity of rear plate
ta = 5; % ambient air temperature in collector (degrees C)
taualfa = 0.85; % collector transmittance-absorbtance product
tfin = 60;  % air inlet temperature (degrees C)
beta = 30;  
...
rhop=2790;  % density of aluminum absorber (kg/m^3 = g/L)
cpp=883;    % specific heat of aluminum plate (J/kg/K)
delp=0.001; % plate thickness (m)
akp=168;    % thermal conductivity of aluminum plate (W/m-K)
rhob=2790;  % density of pink insulation (kg/m^3 = g/L)
cpb=883;    % specific heat of pink insulation (J/kg/K)
delb=0.001; % insulation thickness (m)
akb=168;    % thermal conductivity of pink insulation (W/m-K)
...
if((tyme>=tgtstep)&(ih==0))
ih=1;
gt=gtz+gtstep;
end
if((tyme>=tamdstep)&(im==0))
im=1;
amdot=amdotz+amdstep;
c5=amdot*cpf/(w*dx);
end
s=gt*taualfa;
...
-tb(i,1))-ub*(tb(i,1)-ta));
t(i)=(hcpf*tp(i,2)+hcbf*tb(i,2)+c5*t(i-1))/(hcpf+hcbf+c5);
